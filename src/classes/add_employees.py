...
class AddEmployees(Screen):
    ...
    def create_input_container(self):
        ...
        shift_layout = GridLayout(cols=3, padding=(20, 20, 50, 20), spacing=20)
        shift_layout.size_hint = (1, None)
        shift_layout.height = 200

        self.rounded_background(shift_layout, (0.7, 0.7, 0.7, 0.7))
        shift_layout.bind(size=self.update_container_bg, pos=self.update_container_bg)
        ...
        main_layout.add_widget(Label(text="Enter employee details", size_hint=(1,None), height=60, color=(0,0,0,1)))
        main_layout.add_widget(details_layout)
        main_layout.add_widget(Label(text="Enter employee shift pattern", size_hint=(1,None), height=60, color=(0,0,0,1)))
